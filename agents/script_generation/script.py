...
if not PERPLEXITY_API_KEY:
    raise ValueError("Please set PERPLEXITY_API_KEY in your environment variables.")
...
perplexity_client = AsyncOpenAI(base_url=PERPLEXITY_BASE_URL, api_key=PERPLEXITY_API_KEY)
...
class ResearchAgent:
    def __init__(self):
        self.agent = Agent(
            ...
            """,
            model=OpenAIChatCompletionsModel(model=PERPLEXITY_MODEL, openai_client=perplexity_client),
        )
    ...
    async def generate(self):
        with trace("Script Generation Flow"):
            ...
            generation_prompt = f"""
                ...
                **Important:**  
                - Write only the lines the speaker would say — no descriptions, no labels, no explanation.
